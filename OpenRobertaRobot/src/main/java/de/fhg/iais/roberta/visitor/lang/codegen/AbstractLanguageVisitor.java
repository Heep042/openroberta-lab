...
public abstract class AbstractLanguageVisitor implements ILanguageVisitor<Void> {
    ...
    private Map<Class<?>, IProjectBean> beans = new HashMap<>();

    /**
     * initialize the common language code generator visitor.
     */
    protected AbstractLanguageVisitor(List<ArrayList<Phrase<Void>>> programPhrases, IProjectBean... beans) {
        Assert.isTrue(!programPhrases.isEmpty());
        for ( IProjectBean bean : beans) {
            if (bean != null) {
    ...
    protected <T extends IProjectBean> T getBean(Class<T> beanClass) {
        try {
